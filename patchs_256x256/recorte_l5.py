...
def plot_image_with_patches(image, patch_size=256):
    """
    Función para visualizar los cortes de 256x256 en la primera banda de la imagen.

    :param image: Imagen apilada (multibanda) de tamaño (h, w, c).
    :param patch_size: Tamaño del parche (por defecto 256).
    """
    # Extraer la primera banda de la imagen (la banda 1 será de tamaño (h, w))
    first_band = image[:, :, 0]

    # Crear la figura y el eje para el gráfico
    fig, ax = plt.subplots(figsize=(10, 10))

    # Mostrar la primera banda
    ax.imshow(first_band, cmap='gray')

    # Obtener el tamaño de la imagen
    height, width = first_band.shape

    # Dibujar los rectángulos que representan los cortes de 256x256
    for i in range(0, height, patch_size):
        for j in range(0, width, patch_size):
            # Crear un rectángulo en las posiciones (j, i)
            rect = patches.Rectangle((j, i), patch_size, patch_size, linewidth=1, edgecolor='r', facecolor='none')
            # Añadir el rectángulo al eje
            ax.add_patch(rect)

    # Mostrar la imagen con los cortes superpuestos
    plt.title(f"Cortes de 256x256 en la primera banda")
    plt.show()

def save_patches_to_directory(image, directory, patch_size=256):
    """
    Función para guardar los parches generados en un directorio con el nombre basado en la fila y columna.

    :param image_patches: Lista de parches de tamaño (256, 256, 6).
    :param directory: Ruta del directorio donde se guardarán los parches.
    :param original_image_shape: Tamaño de la imagen original (h, w, c).
    :param patch_size: Tamaño del parche (por defecto 256x256).
    """
    ...
    for i, y in enumerate(range(0, height, patch_size)):
        for j, x in enumerate(range(0, width, parche_size)):
            ...
            nombre_parche = f"{directory}_{i+1:03d}_{j+1:03d}.TIF"
            ruta_parche = os.path.join(directory, nombre_parche)

            with rasterio.open(ruta_parche, 'w', **profile) as dst:
                dst.write(parche)

    """
    # Recorrer los parches
    patch_idx = 0
    for i in range(0, height, patch_size):
        for j in range(0, width, patch_size):
            # Definir el nombre del archivo
            patch_filename = os.path.join(directory, f"patch_row_{i+1:03d}_col_{j+1:03d}.tif")

            # Obtener el parche correspondiente
            patch = image_patches[patch_idx]
            patch_idx += 1

            # Definir el transformado espacial para el parche (puedes ajustar la resolución según sea necesario)
            #transform = from_origin(j, i, 30, 30)  # Suponiendo 30m x 30m por píxel (ajustar si necesario)

            # Guardar el parche como un archivo GeoTIFF
            with rasterio.open(
                patch_filename, 'w',
                driver='GTiff',
                height=patch.shape[0],
                width=patch.shape[1],
                count=patch.shape[2],  # Número de bandas (6 en este caso)
                dtype=patch.dtype,
                crs='EPSG:32618'  # Ajustar el CRS si es necesario
            ) as dst:
...
output_directory ="/home/cornelius/Documentos/Glaciar_dataset_C2_L1/landsat5_C2L1/patch_001071/Grid_patch_001071/"
